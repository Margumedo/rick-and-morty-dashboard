
import { CustomIcon } from "@/components/CustomIcon";
import { CardSumamaryProps } from "./CardSummary.types";
import CustomTooltip from "@/components/CustomTooltip/CustomTooltip";
import { cn } from "@/lib/utils";
import { MoveDownRight, TrendingUp } from "lucide-react";



export function CardSummary(props: CardSumamaryProps) {
    const { average, icon, title, tootltipText, total } = props

    return (
        <div className="shadow-sm bg-background rounded-lg p-5 py-3 hover:shadow-lg transition" >
            <div className="flex justify-between ">
                <div className="flex gap-3 items-center">
                    <CustomIcon icon={icon} />
                    {title}
                </div>
                <CustomTooltip content={tootltipText} />
            </div>
            <div className="flex gap-4 mt-2 md:mt-4">
                <p className="text-2xl" >{total}</p>
                <div className={cn(`flex items-center gap-1 px-2 text-xs text-white rounded-lg h-[20px] bg-black dark:bg-secondary`)}>
                    {average}%

                    {average < 20 && (
                        <MoveDownRight strokeWidth={1} className="h-4 w-4" />
                    )}

                    {average > 20 && average < 70 && (
                        <MoveDownRight strokeWidth={1} className="h-4 w-4" />
                    )}

                    {average > 70 && average < 100 && (
                        <TrendingUp strokeWidth={1} className="h-4 w-4" />
                    )}
                </div>

            </div>
        </div>
    )
}
